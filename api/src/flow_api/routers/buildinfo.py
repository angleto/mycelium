...
``FLOW_VERSION``, ``FLOW_GIT_SHA``, ``FLOW_BUILD_AT``. All optional —
...
async def get_buildinfo() -> BuildInfoOut:
    version = os.environ.get("FLOW_VERSION", "dev") or "dev"
    sha = os.environ.get("FLOW_GIT_SHA", "") or ""
    built_at = os.environ.get("FLOW_BUILD_AT", "") or ""
    return BuildInfoOut(
        version=version,
        git_sha=sha,
