...
async def _member_actor(
    org_id: uuid.UUID, owner_id: uuid.UUID, name: str, kind: str
) -> ScenarioActor:
    ...
    async with tenant_session(str(org_id), str(owner_id)) as s:
        await add_member(s, org_id=org_id, actor_id=owner_id, email=email, role="member")
...
def _parse() -> tuple[argparse.Namespace, Path]:
    ...
    ap.add_argument("--concurrency-per-level", type=int, default=30)
    args = ap.parse_args()
...
async def main(args: argparse.Namespace, out_dir: Path) -> None:
    ...
    runner = ScenarioRunner(org_id=org_id, k=args.k, seed=args.seed)
    ...
    if not args.skip_concurrency:
        ...
        await run_concurrency_latency(
            ...
            per_level=args.concurrency_per_level,
    ...
    for row in manifest["concurrency_latency"]:
        print(
            f"  concurrency {row['concurrent_clients']}: p50={row['p50_ms']}ms "
            f"p95={row['p95_ms']}ms ({row['hardware']})"
        )
