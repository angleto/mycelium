...
export const it: Catalog = {
  ...
  sdi: {
    ...
    codeMissing:
      'Non impostato. La trasmissione tramite il canale accreditato viene rifiutata finche\u0301 non lo compili: senza, FatturaPA non ha dove mettere il trasmittente.',
    codeLooksLikeVat:
  ...
  revisions: {
    ...
    fields: {
      ...
      part_create: 'Parte {{n}}: aggiunta',
      part_trash: 'Parte {{n}}: rimossa',
      part_restore: 'Parte {{n}}: ripristinata',
      part_purge: 'Parte {{n}}: eliminata definitivamente',
      parts_reorder: 'Parti riordinate',
      parts_merge_in: 'Parti unite da un\'altra nota',
      parts_merge_out: 'Parti spostate in un\'altra nota',
      _create: 'Creazione',
  ...
  notes: {
    ...
    parts: {
      ...
      confirmDelete: 'Rimuovere questa parte? Potrai annullare.',
      removeHint: 'Rimuovi questa parte',
      trashed: 'Parte rimossa.',
      undoRemove: 'Annulla',
      moveUp: 'Sposta su',
      moveDown: 'Sposta giù',
  ...
  paymentConnectors: {
    ...
    empty: 'Nessun connettore di pagamento per questo emittente.',
    create: 'Nuovo connettore',
    confirmCreate: 'Crea',
    cancel: 'Annulla',
    save: 'Salva',
    edit: 'Impostazioni',
    label: 'Etichetta',
    labelHint: 'Testo libero, unico dentro questo emittente: così "Stripe live" e "Stripe test" possono convivere sullo stesso cedente.',
    provider: 'Provider',
    signingSecret: 'Chiave di firma',
    signingSecretHint: 'Per Stripe incolla il whsec_… che la dashboard Stripe mostra quando crei l’endpoint webhook: dobbiamo ricalcolare la firma prodotta da Stripe, quindi non possiamo generarla noi. Lascia vuoto solo per il contratto nativo mycelium, dove l’autorità è Mycelium e la chiave viene generata qui (mostrata una sola volta, qui sotto).',
    withApiKey: 'Genera anche una chiave API di ingresso',
    withApiKeyHint: 'Secondo fattore opzionale: il chiamante la rimanda nell’header X-Connector-Api-Key. La firma resta comunque obbligatoria.',
    enabled: 'attivo',
    disabled: 'non attivo',
    enable: 'Attiva',
    disable: 'Disattiva',
    enableTransmitConfirm: 'Attivare questo connettore? Le fatture o le note di credito verranno composte E trasmesse allo SdI in automatico, senza altra revisione.',
    invoiceMode: 'Fatture',
    creditNoteMode: 'Note di credito',
    modeTransmit: 'componi e trasmetti',
    modeDraft: 'componi in bozza',
    modeDryRun: 'prova a vuoto: compone e valida, non spedisce',
    modeOff: 'metti in attesa (gestione manuale)',
    modeHint: 'Due interruttori indipendenti: automatizzare le fatture tenendo manuali le note di credito è una scelta normale. In bozza il documento viene composto e aspetta che sia tu a trasmetterlo; gli eventi messi in attesa finiscono nella quarantena qui sotto.',
    emissionEvent: 'Evento che emette',
    emissionEventHint: 'Un solo evento del provider genera un documento. Stripe ne emette diversi per lo stesso incasso (un invoice.paid è anche un charge.succeeded), quindi gli altri servono soltanto a riconciliare il pagamento.',
    vatRate: 'Aliquota IVA predefinita (%)',
    vatRateHint: 'Usata quando il provider non riporta imposta sulla riga. Se vuoto si applica il valore predefinito del regime fiscale dell’emittente.',
    series: 'Sezionale',
    ...
    refundEvent: 'Evento di rimborso',
    refundEventHint: 'Stripe annuncia lo stesso rimborso due volte, come refund.created e come charge.refunded, e i due non si deduplicano fra loro quando il payload non espande refunds.data. Il connettore ne ascolta uno solo: sottoscrivi nella dashboard esattamente quello scelto qui, l’altro verrebbe comunque ignorato (con motivo registrato). refund.created è la scelta giusta salvo endpoint storici che ricevono solo charge.refunded.',
    setupTitle: 'Come configurare {{provider}}',
    setupStepEndpoint: 'Nella dashboard del provider aggiungi un endpoint webhook con l’indirizzo qui sopra.',
    setupStepEvents: 'Abilita esattamente questi eventi:',
    setupStepEndpointNative: 'Comunica questo indirizzo a chi implementa il contratto: è l’endpoint a cui deve inviare gli eventi firmati.',
    setupStepSecretNative: 'Consegna al mittente la chiave di firma (generata qui alla creazione, oppure una concordata con lui) e il contratto: docs/payment-connector-contract.md. La firma è HMAC-SHA256 su {timestamp}.{corpo}, la stessa costruzione di Stripe.',
    setupStepSecret: 'Copia la chiave di firma dell’endpoint e incollala qui (bottone Ruota chiave di firma). Finché non combacia, ogni consegna viene rifiutata come firma non valida.',
    setupStepDryRun: 'Tieni la modalità fattura su “prova a vuoto” finché non hai confrontato gli XML generati: in quella modalità non viene spedito nulla allo SdI e non si consuma alcun numero.',
    copyEvents: 'Copia elenco',
    promote: 'Rendi inviabile',
    promoteConfirm: 'Trasformare questa fattura di prova in un documento reale? Esce dall’archivio e torna fra le bozze inviabili, e la rivendicazione del pagamento passa dall’universo di prova a quello reale, così una riconsegna dello stesso evento non genererà una seconda fattura. Il numero fiscale viene assegnato solo quando la invii davvero. L’operazione viene rifiutata se un documento reale copre già questo pagamento.',
    promoted: 'Fattura promossa: ora è una bozza inviabile in elenco fatture.',
    dryRunBadge: 'non inviata: prova a vuoto',
    vendorOrderHint: 'Ordine delle operazioni: salva il connettore SENZA chiave, copia l’URL che compare, registralo come endpoint webhook su Stripe, e solo allora Stripe ti mostra il whsec_… da incollare qui. È obbligato dal provider: l’URL contiene l’id di questo connettore, quindi il connettore deve esistere prima. Finché la chiave manca il connettore non è attivabile e ogni consegna viene rifiutata.',
    createdNextStep: 'Connettore creato. Ora registra questo indirizzo come endpoint webhook nel provider: è da lì che otterrai la chiave di firma da incollare qui.',
    secretMissing: 'chiave di firma mancante',
    secretMissingHint: 'Questo connettore non può ancora verificare nulla e non è attivabile. Registra l’URL qui sopra nel provider e incolla la chiave che ti mostra, con “Ruota chiave di firma”.',
    triageMovedHint: 'Gli eventi in quarantena e il registro delle consegne si consultano da Fatture, non da qui: sono lavoro quotidiano sui documenti, non configurazione.',
    triageTitle: 'Incassi automatici',
    triageHint: 'Cosa hanno prodotto i connettori di pagamento e cosa hanno rifiutato: eventi in attesa dei dati di fatturazione, eventi che richiedono una decisione, e il registro delle consegne in ingresso.',
    ...
    showPayload: 'Dati ricevuti',
    reason: {
      client_billing_data_missing: 'Il cliente non ha ancora i dati di fatturazione',
      client_rejected: 'Il cliente non è stato accettato dall’anagrafica',
      connector_disabled: 'Connettore non attivo quando è arrivato l’evento',
      connector_gone: 'Connettore revocato',
      credit_note_manual: 'Note di credito impostate su manuale',
      dry_run_credit_note_unsupported: 'In prova a vuoto le note di credito non sono supportate',
      dry_run_invalid_document: 'Il documento di prova non supera la validazione',
      emission_event_not_mapped: 'Evento di emissione non gestito',
      event_type_not_mapped: 'Tipo di evento non gestito dal connettore',
      invoice_mode_manual: 'Emissione fatture impostata su manuale',
      parent_not_transmitted: 'La fattura di riferimento non è ancora stata trasmessa',
      parent_rejected: 'La fattura di riferimento è stata scartata',
      payload_invalid: 'Il contenuto ricevuto non è leggibile',
      refund_event_not_selected: 'Annuncio di rimborso diverso da quello configurato',
      payment_sync_off: 'Sincronizzazione pagamenti disattivata',
      payment_without_invoice: 'Nessuna nostra fattura corrisponde a questo pagamento',
    },
    field: {
      vat_number: 'partita IVA',
      tax_code: 'codice fiscale',
    ...
    outcomeLabel: {
      accepted: 'Accettata',
      duplicate: 'Duplicata (già ricevuta)',
      signature_invalid: 'Firma non valida',
      disabled: 'Connettore non attivo',
      payload_invalid: 'Contenuto non leggibile',
      too_large: 'Corpo troppo grande',
    },
    noBillingDataHint: 'Se molti pagamenti restano qui, controlla che in Stripe siano sottoscritti customer.created e customer.updated: Stripe non ha un campo per il codice destinatario, quindi vive nei metadati del cliente, e i metadati del cliente viaggiano SOLO su quegli eventi (il payload di una fattura Stripe non è espandibile e nomina il cliente solo per id). Appena arriva un evento customer.* con i dati mancanti, questi pagamenti si sbloccano da soli. Puoi verificare cosa è arrivato cliccando l’id dell’evento.',
    vatPricing: 'Importi e IVA',
    vatPricingLabel: {
      auto: 'Automatico (consigliato)',
      gross: 'Sempre IVA inclusa (forza)',
      net: 'Sempre imponibile (forza)',
    },
    vatPricingHint: 'Automatico: se il provider dichiara il trattamento IVA (Stripe lo fa quando calcola le imposte) quella dichiarazione vince, perché descrive denaro realmente incassato. Se non dichiara nulla, l’importo viene letto come TOTALE già comprensivo di IVA: è denaro già incassato, quindi aggiungerci l’IVA sopra fatturerebbe più di quanto il cliente ha pagato. Le due opzioni "forza" ignorano quello che dice il provider e servono solo se sai che le sue impostazioni fiscali sono sbagliate. Un importo di charge o payment_intent resta comunque lordo: è ciò che la carta ha pagato.',
    ...
    purposePaymentSync: 'marca la fattura come pagata',
    purposeUnknown: 'richiesto dal connettore',
    purposeOptional: '(facoltativo)',
    setupNoExtraEvents: 'Non abilitare altri eventi di rimborso oltre a {{refundEvent}}: i due annunci descrivono lo stesso rimborso e abilitarli entrambi produrrebbe due note di credito.',
    show: 'Mostra',
    hide: 'Nascondi',
    secretMint: 'Genera la chiave qui',
    secretProvide: 'Inserisci una chiave mia',
    secretPlaceholder: 'almeno 16 caratteri',
    nativeSecretHint: 'Sul contratto nativo l’autorità è Mycelium, quindi la chiave può essere generata qui (mostrata una sola volta) oppure puoi incollarne una tua se il mittente ne usa già una. In entrambi i casi non viene mai mostrata di nuovo: è cifrata a riposo e le consegne si validano ricalcolando la firma.',
    nativeEventsHint: 'Il contratto nativo definisce un evento per esito (invoice.issue, invoice.credit, invoice.payment), quindi non c’è nulla da scegliere: l’elenco da implementare è qui sotto, nella guida. I dati fiscali del cliente viaggiano dentro l’evento di emissione, quindi non serve un canale separato per l’anagrafica.',
    rotateSigningVendorHint: 'Incolla la nuova chiave che la dashboard del provider mostra dopo la rotazione. Non possiamo generarla noi: la firma la calcola il provider, quindi una chiave inventata farebbe rifiutare ogni consegna. La chiave attuale continua a validare per la finestra di tolleranza, così gli eventi già in coda di retry non si perdono.',
    rotateSigningNativeHint: 'Puoi incollare una chiave concordata col mittente oppure farne generare una nuova. La precedente continua a validare per la finestra di tolleranza, così gli eventi già in coda di retry non si perdono.',
    webhookUrlHint: 'Incolla questo indirizzo nella dashboard del provider come URL dell’endpoint. È un indirizzo di instradamento, non una credenziale: l’autorità viene dalla firma.',
    webhookUrlUnset: 'Questo indirizzo non è assoluto: l’installazione non ha un URL pubblico di base configurato, quindi il provider non avrebbe dove inviare la richiesta. Fallo impostare prima di incollarlo da qualche parte.',
    copy: 'Copia',
    copyUrl: 'Copia URL',
    dismiss: 'Chiudi',
    secretWarning: 'Copialo adesso: viene mostrato una sola volta e non è più recuperabile.',
    secretShown: 'Chiave di firma',
    apiKeyShown: 'Chiave API di ingresso',
    apiKeyArmed: 'chiave API attiva',
    lastEvent: 'Ultimo evento',
    never: 'mai',
    rotateSigning: 'Ruota chiave di firma',
    rotateApiKey: 'Ruota chiave API',
    rotateApiKeyConfirm: 'Ruotare la chiave API di ingresso? Quella attuale resta valida per la finestra di tolleranza, poi smette di funzionare.',
    clearApiKey: 'Rimuovi chiave API',
    clearApiKeyConfirm: 'Rimuovere la chiave API di ingresso? Il connettore autentica solo con la firma.',
    revoke: 'Revoca',
    revokeConfirm: 'Revocare questo connettore? Smette subito di accettare eventi; lo storico resta consultabile.',
    revoked: 'revocato',
    delete: 'Elimina',
    purgeConfirm: 'Eliminare definitivamente questo connettore revocato? Se ne va anche lo storico di eventi e consegne; le fatture già emesse restano intatte.',
    quarantine: 'Quarantena',
    quarantineHint: 'Eventi arrivati, verificati, che non sono diventati una fattura. "In attesa dei dati" tocca al cliente, non a te: si riarmano da soli appena il cliente inserisce quello che manca. "Richiede attenzione" richiede una tua decisione. "Rinunciato" ha esaurito i tentativi.',
    quarantineEmpty: 'Niente da vedere qui.',
    statusNoBillingData: 'In attesa dei dati di fatturazione',
    statusNeedsAttention: 'In attesa di te',
    ...
    eventType: 'Evento',
    providerEventId: 'Id evento provider',
    errorSlug: 'Motivo',
    attempts: 'Tentativi',
    createdAt: 'Ricevuto',
    retry: 'Riprova',
