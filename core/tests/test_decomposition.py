...
async def test_distill_accepts_caller_supplied_text() -> None:
    """dec45ebc + audit A-2: an external MCP caller can write the humus atom
    with its OWN strong model. The caller text is persisted verbatim (stripped)
    and Mycelium runs/meters NO internal LLM call. But because the content is
    UNVERIFIED it is hardened: provenance is namespaced ``external:`` (no
    masquerading as an internal model) and the atom is born ``proposed`` (gated
    out of retrieval until a human approves it). Idempotent."""
    org, user = await _org()
    async with tenant_session(str(org), str(user)) as s:
        await _seed_billing(s, org, user)
        source = await nt.create_note(
        ...
        # Provenance is namespaced so it cannot pose as an internal model.
        assert res.model_id == "external:claude-opus-4-8"
        distilled = (
            await s.execute(select(Note).where(Note.id == res.distilled_note_id))
        ).scalar_one()
        assert distilled.humus_kind == "distillation"
        assert distilled.humus_flag is True
        assert distilled.origin_model_id == "external:claude-opus-4-8"
        # Born gated: withheld from every retrieval surface until approved.
        assert distilled.review_state == "proposed"
...
async def test_distill_external_text_is_length_capped() -> None:
    """Audit A-2: the external path skips the internal LLM (and its implicit
    length bound), so an over-cap caller text is rejected rather than written as
    an unbounded blob."""
    org, user = await _org()
    async with tenant_session(str(org), str(user)) as s:
        source = await nt.create_note(
            ...
            kind=NoteKind.text,
            title="big",
            text="a finished thought worth distilling",
        )
        with pytest.raises(DomainError):
            await decomp.distill_note(
                s,
                org_id=org,
                actor_id=user,
                note_id=source.id,
                distilled_text="x" * (decomp._MAX_EXTERNAL_DISTILL_CHARS + 1),
            )
