...
async def test_grader_floor_helper_off_sentinels() -> None:
    ...
    async with tenant_session(str(org), str(user)) as s:
        ...
        # An in-domain value passes through unchanged.
        await s.execute(
            update(Organization)
            .where(Organization.id == org)
            .values(settings={mem.GRADER_MIN_RRF_KEY: 0.02})
        )
        assert await mem.grader_min_rrf_floor(s, org) == 0.02
        # A value above the fused-RRF domain is CLAMPED to GRADER_MIN_RRF_MAX,
        # not accepted verbatim (audit A-1: 0.25 would otherwise abstain every
        # query). The [0,1] footgun is closed.
        await s.execute(
            update(Organization)
            .where(Organization.id == org)
...
async def test_grader_min_rerank_score_floor_helper_off_sentinels() -> None:
    ...
    async with tenant_session(str(org), str(user)) as s:
        assert await mem.grader_min_rerank_score_floor(s, org) is None  # absent
        await s.execute(
            update(Organization)
            .where(Organization.id == org)
            .values(settings={mem.GRADER_MIN_RERANK_SCORE_KEY: 0.0})
        )
        assert await mem.grader_min_rerank_score_floor(s, org) is None  # zero = off
        await s.execute(
            update(Organization)
            .where(Organization.id == org)
            .values(settings={mem.GRADER_MIN_RERANK_SCORE_KEY: "junk"})
        )
        assert await mem.grader_min_rerank_score_floor(s, org) is None  # malformed
        await s.execute(
            update(Organization)
            .where(Organization.id == org)
            .values(settings={mem.GRADER_MIN_RERANK_SCORE_KEY: 0.7})
        )
        assert await mem.grader_min_rerank_score_floor(s, org) == 0.7  # in-domain prob
        # Above the [0, 1] probability domain -> clamped to 1.0, not verbatim.
        await s.execute(
            update(Organization)
            .where(Organization.id == org)
            .values(settings={mem.GRADER_MIN_RERANK_SCORE_KEY: 4.2})
        )
        assert await mem.grader_min_rerank_score_floor(s, org) == 1.0
