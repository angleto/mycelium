...
async def test_note_projection_is_enriched_and_projectable() -> None:
    ...
    listed = (await list_notes(token=token, org_id=org))["items"]
    row = next(r for r in listed if r["id"] == n["id"])
    assert row["maturity"] == "seed"

    # The keep-filter now actually selects the new column.
    projected = (await list_notes(token=token, org_id=org, fields=["maturity"]))["items"]
...
async def test_list_notes_filters_by_maturity() -> None:
    ...
    dormant = (await list_notes(token=token, org_id=org, maturity="dormant"))["items"]
    assert any(r["id"] == n["id"] for r in dormant)
    assert all(r["maturity"] == "dormant" for r in dormant)
    # A different stage excludes it.
    seeds = (await list_notes(token=token, org_id=org, maturity="seed"))["items"]
...
async def test_list_notes_by_task_returns_enriched_in_one_call() -> None:
    ...
    rows = (await list_notes(token=token, org_id=org, task_id=task["id"]))["items"]
    ...
    assert (await list_notes(token=token, org_id=org, task_id=str(uuid.uuid4())))["items"] == []
...
async def test_proposed_notes_excluded_even_via_task_path() -> None:
    ...
    assert not any(r["id"] == n["id"] for r in (await list_notes(token=token, org_id=org))["items"])
    assert not any(
        r["id"] == n["id"]
        for r in (await list_notes(token=token, org_id=org, task_id=task["id"]))["items"]
    )
