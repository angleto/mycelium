...
def _smtp_settings(**over: object) -> Settings:
    base: dict[str, object] = {
        ...
        "smtp_from": "Flow <no-reply@flow.leto.blue>",
        "smtp_starttls": True,
    }
    base.update(over)
...
async def test_smtp_send_uses_starttls_login_and_sendmessage(
    ...
    mailer = SmtpMailer(
        ...
        username="tem-user",
        password="tem-pass",
        sender="Flow <no-reply@flow.leto.blue>",
        starttls=True,
    )
    ...
    assert fake.sent["From"] == "Flow <no-reply@flow.leto.blue>"
...
async def test_smtp_send_skips_login_without_username_and_starttls_off(
    ...
    assert fake.sent is not None
    assert fake.sent["From"] == "ops@flow.leto.blue"
