...
When a deployment switches ``FLOW_ATTACHMENT_STORE`` from ``pg`` to
...
Run: ``python -m flow_core.migrate_attachments`` (backend image in the
...
from flow_core.attachment_store import AttachmentStore, get_attachment_store
