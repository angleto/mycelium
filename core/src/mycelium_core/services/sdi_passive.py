...
The endpoint that receives this lives in ``mycelium_sdi_inbound.app``: it routes
...
async def ingest_receiver_dt(parsed) -> ReceivedInvoice | None:  # type: ignore[no-untyped-def]
    ...
    from mycelium_core.models.invoice import BuyerVerdict
...
async def ingest_receiver_notification(raw: bytes) -> ReceivedInvoice | None:
    ...
    from mycelium_core.services.sdi_notification_xsd import validate_sdi_notification
