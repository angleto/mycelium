...
Wired once at startup in ``mycelium_api.app`` (lifespan) and
``mycelium_worker.main`` via ``set_sender_override`` -- the same single-seam
pattern as ``set_mailer``. The ASGI test transport does not run the
lifespan and ``mycelium_worker.main`` is never imported by the test suite, so
...
from mycelium_core.services.notifications_webpush import WebPushNotificationSender
