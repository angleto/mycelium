    ...
    async def run(
        ...
        for c in candidates:
            ...
            for stage_name, rank in c.scores_by_stage.items():
                if stage_name == self.name:  # never fold a prior fused score back in
                    continue
                weight = self.weights.get(stage_name, 1.0)
                fused += weight / (self.k + rank)
            c.score = fused
            # Surface the fused score under the stage name so an observability
            # caller finds the documented ``rrf`` key (the per-branch entries
            # are 1-based RANKS, lower = better; this one is the fused SCORE,
            # higher = better). The reranker, if it runs, overwrites it.
            c.scores_by_stage[self.name] = fused
        return candidates
