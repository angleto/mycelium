...
to receive a message from that user. So binding a Mycelium account to a
...
multiple workspaces opens Mycelium on the same Telegram chat regardless
...
class TelegramLink(TimestampMixin, VersionMixin, Base):
    """One Telegram identity per Mycelium user. ``user_id`` is the PK so
    ...
    __table_args__ = (
        # Stops two Mycelium users from racing onto the same Telegram chat
