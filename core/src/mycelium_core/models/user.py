...
from sqlalchemy import DateTime, LargeBinary, SmallInteger, String, Text, text
...
class User(UUIDPKMixin, TimestampMixin, Base):
    ...
    # Generated mycelium-QR avatar: a real, scannable vCard QR rendered as a
    # mycelial network (migration 0072). Bytes live in a DEFERRED column so
    # the many /auth/me + enumeration queries never pull them; the avatar
    # endpoint loads them with an explicit undeferred select. ``avatar_seed``
    # + the two colors reproduce the styling deterministically;
    # ``avatar_updated_at`` busts the cached <img>.
    ...
    avatar_bg: Mapped[str | None] = mapped_column(String(9), nullable=True)
    avatar_net: Mapped[str | None] = mapped_column(String(9), nullable=True)
    avatar_updated_at: Mapped[datetime.datetime | None] = mapped_column(
        DateTime(timezone=True), nullable=True
    )
