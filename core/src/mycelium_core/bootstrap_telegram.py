...
Run: ``python -m mycelium_core.bootstrap_telegram`` from the backend
...
async def run() -> str:
    ...
    if not settings.telegram_configured:
        raise SystemExit(
            "Telegram bot is not configured: MYCELIUM_TELEGRAM_BOT_TOKEN, "
            "MYCELIUM_TELEGRAM_BOT_USERNAME and MYCELIUM_TELEGRAM_WEBHOOK_SECRET "
            "must all be set"
        )
    ...
    if not base:
        raise SystemExit(
            ...
            "MYCELIUM_TELEGRAM_WEBHOOK_BASE_URL or MYCELIUM_FRONTEND_BASE_URL"
        )
