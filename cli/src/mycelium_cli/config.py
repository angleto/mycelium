...
``~/Library/Application Support/mycelium`` on macOS): a dotfile-friendly
``~/.config/mycelium/`` matches what ``gh``, ``aws``, ``kubectl`` and friends
use across macOS and Linux and is what a tmux-resident user expects.

``MYCELIUM_CONFIG_DIR`` overrides everything (used in tests and for ad-hoc
...
def config_dir() -> Path:
    env_override = os.environ.get("MYCELIUM_CONFIG_DIR")
    if env_override:
        return Path(env_override).expanduser()
    base = os.environ.get("XDG_CONFIG_HOME") or str(Path.home() / ".config")
    return Path(base) / "mycelium"
...
class Profile:
    base_url: str = "https://mycelium.xeno.garden/api"
...
def load_config() -> Config:
    ...
    for name, body in raw_profiles.items():
        ...
        profiles[str(name)] = Profile(
            base_url=str(body.get("base_url", "https://mycelium.xeno.garden/api")),
