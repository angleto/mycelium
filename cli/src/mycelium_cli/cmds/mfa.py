"""``mycelium auth mfa`` — TOTP enrolment, status, disable.

Setup flow:
  1. ``mycelium auth mfa setup`` prints the otpauth URL, an ASCII QR code,
     and the base32 secret. Scan the QR with your authenticator (or
     paste the secret manually).
  2. ``mycelium auth mfa activate <CODE>`` confirms a fresh TOTP code and
     enables MFA; the response includes one-time backup codes — save
     them.
  3. From the next login onward, ``mycelium auth login`` will prompt for
...
from mycelium_cli.ui import emit_json, json_mode, out, success
...
def status() -> None:
    ...
    if st.get("pending"):
        out().print("[yellow]pending activation[/yellow] (run: mycelium auth mfa activate <CODE>)")
    if enabled:
...
def setup(
    ...
    out().print("\nThen confirm with: [bold]mycelium auth mfa activate <6-digit code>[/bold]")
