"""``flow auth`` — login (mint PAT), logout, whoami, status."""
...
def login(
    ...
    token_name: str | None = typer.Option(
        None,
        "--token-name",
        help="Name for the new agent token (defaults to flow-cli@<hostname>).",
    ),
    ...
    with jwt_client(base_url, jwt) as c:
        ...
        body: dict[str, Any] = {
            "name": token_name or f"flow-cli@{socket.gethostname()}",
            "scope": "cli",
        }
        if ttl_days > 0:
...
def whoami(
    ...
    cred = load_credential(profile)
    if cred is None:
        fail(f"No credential stored for profile '{profile}'.", hint="Run: flow auth login")
        raise typer.Exit(1)
    if json_mode():
        emit_json(
            {
...
def status(profile: str = typer.Option("default", "--profile")) -> None:
    ...
    if cred is None:
        fail(f"profile '{profile}' is not logged in.", hint="Run: flow auth login")
        raise typer.Exit(1)
    ok = True
    try:
