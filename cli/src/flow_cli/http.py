"""Thin HTTP client wrapped around ``httpx`` with Flow-specific defaults.
...
from flow_cli.credentials import Credential
...
def _build_client(
    ...
    *,
    ...
    headers: dict[str, str] = {
        ...
        "User-Agent": f"flow-cli/{__version__}",
    }
    if bearer:
...
def raise_for_response(resp: httpx.Response) -> None:
    ...
    Flow's error contract surfaces ``{"detail": {"code": "...",
    ...
    if resp.status_code in (401, 403):
        raise AuthRequired(msg, hint="Run: flow auth login")
    raise CLIError(msg)
